...
<template>
  ...
  >
    <div class="flex items-center justify-center  flex-1">
        <!-- upload design -->
      <div class="flex flex-col items-center justify-center gap-4 h-screen max-w-[350px]">
        <img :src="image" />
        <input
          type="text"
          v-model="image"
    ...
    <form class="flex flex-col items-start justify-start flex-1 gap-8">
      <div class="flex flex-col gap-4 items-start justify-start w-full">
        <!-- design title -->
        <AdminInputData label="Design Title" placeholder="e.g.Abstract watercolor poster" v-model="title"/>
        <!-- description -->
        ...
        <!-- Tags -->
        <AdminInputData label="Tags" placeholder="e.g.#AbstractArt #WallPrint #HomeDecor " v-model="tags"/>
      </div>
      <!-- action btns -->
