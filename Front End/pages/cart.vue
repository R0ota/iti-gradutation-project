...
const totalPrice = computed(() => {
  ...
  const shipping = selectedShipping.value ? selectedShipping.value.price : 0;
    ...
    <div class="flex lg:flex-row flex-col gap-8 w-full md:flex-row">
      ...
      >
        ...
        <div v-if="cart.items.length">
          <div v-for="item in cart.items" :key="item.id" class="w-full">
            <Cart>
              <ProductData :img="item.image" />
            </Cart>
          </div>
        </div>
        <p v-else class="text-stone-400 text-sm">Your cart is empty.</p>
      </div>
        ...
        <div class="flex flex-col gap-6 lg:w-95 w-80 items-start">
          <div class="flex flex-col lg:gap-4 gap-3 w-full  lg:items-start">
            ...
            >
              ...
              >
                <div
                  class="lg:w-8 w-5 lg:h-8 w-5 flex justify-center items-center"
                >
